...
/**
 * @file gemm_register.cu
 * @brief Register Tiling (2D Thread Tiling) GEMM
 *
 * @algorithm
 * - Extends Shared Memory Tiling by adding **Thread-Level Tiling**.
 * - Each thread computes a 2D sub-tile of size TM x TN (e.g., 8x8) entirely in its private Registers.
 * - **Extreme Data Reuse**: Data is loaded from Shared Memory into Registers (`fragA`, `fragB`). An 8x8 thread tile does 64 FMA operations using only 8 loads from As and 8 loads from Bs.
 * - **Compute Bound Achieved**: This ratio dramatically hides Shared Memory latency and saturates the CUDA cores.
 *
 * @bottleneck
 * - **Register Pressure**: High register usage reduces SM Occupancy (fewer active warps to hide global memory latency).
 */
